...
class GuardAgainstHistoryTest(unittest.TestCase):
    ...
    # This fork's history up to here predates the guard: 25 of its own commits carry
    # an assistant co-author trailer, and rewriting published history to drop them
    # would invalidate every clone and release tag of the fork for a cosmetic gain.
    # Everything committed AFTER this point must be clean -- which is what the
    # commit-msg hook enforces locally and what CI checks on a pull request.
    BASELINE = "24b1b6473c204d81ccd4c8ce634db9da4dbb3268"

    def test_the_shipped_history_is_clean(self) -> None:
        # Doubles as the answer to "is the history we control already tainted".
        offenders = [
            message
            for message in self._messages(f"{self.BASELINE}..HEAD")
            if _check(message).returncode != 0
        ]
