...
async def async_setup_entry(
    ...
    for appliance_id, data in data_map.items():
        ...
        if app_type == APPLIANCE_HW:
            # Heat pump water heater: NO select. The operating mode lives on the
            # water_heater entity (operation_mode + away), which reads and writes the
            # same startProgram program; a parallel select was a second surface for
            # the same control and was retired in v5.21.0 (the registry entry is
            # cleaned up by _remove_legacy_entities in __init__).
            continue
