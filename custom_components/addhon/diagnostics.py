...
def _mapped_sets(
    ...
    try:
        ...
        DATES = {}
        _note_missing_registry("date", unavailable)
    try:
        from .time import TIMES
    except Exception:  # noqa: BLE001 - a dump must degrade, never raise
        TIMES = {}
        _note_missing_registry("time", unavailable)
    try:
    ...
    for desc in DATES.get(app_type, ()):
        ...
        mapped_params.add(desc.param)
        sources[f"date.{desc.key}"] = _source_row(
            read=_read_chain(desc.param), write=[desc.param]
        )
    # The schedule windows (HW/WH) work the same way: one settings parameter that is
    # both the reading and the write target, so the row names it on both halves.
    for desc in TIMES.get(app_type, ()):
        mapped_attrs.add(desc.param)
        mapped_params.add(desc.param)
        sources[f"time.{desc.key}"] = _source_row(
            read=_read_chain(desc.param), write=[desc.param]
        )
